...
class ZIPArchive {
    ...
    private func parseEntries() throws {
        ...
        for i in stride(from: searchData.count - 22, through: 0, by: -1) {
            if searchData[i] == 0x50 && searchData[i + 1] == 0x4b && searchData[i + 2] == 0x05
                && searchData[i + 3] == 0x06
            {
                eocdOffset = i
                break
        ...
        while true {
            ...
            if sig != 0x0201_4b50 { break }  // Central Directory File Header
    ...
    func extract(_ entry: Entry, to url: URL) throws {
        ...
        let sigData = handle.readData(ofLength: 4)
        guard readUInt32(from: sigData, at: 0) == 0x0403_4b50 else {
            throw NSError(
                domain: "ZIPArchive",
                code: 3,
